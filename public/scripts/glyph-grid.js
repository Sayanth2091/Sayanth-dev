// Public copy of glyph grid (lightweight)
(function(){
  function initGlyphGrid() {
    ...
    function updateGlyphs() {
      ...
      for (let i = 0; i < cells.length; i++) {
        ...
        if (Math.random() > chance) {
          ...
          setTimeout(() => { c.element.textContent = ''; }, life);
        }
      }
    }
    function matrixFall() {
    ...
    build();
    setInterval(updateGlyphs, 120);
    setInterval(matrixFall, 260);
  }
  function readyOrWait(cb){ if (document.body && document.body.classList.contains('app-loading')) { window.addEventListener('app:ready', cb, { once:true }); } else cb(); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => readyOrWait(initGlyphGrid), { once: true }); else readyOrWait(initGlyphGrid);
})();
